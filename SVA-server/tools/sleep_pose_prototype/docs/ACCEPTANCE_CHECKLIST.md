...
Python 原型、ONNX 导出、C++ 推理、睡岗状态机和单元测试已经跑通。首轮 9 段公开素材用于验证程序结构和阈值边界；`P04_hand_sleeping.mp4` 仍是需要由目标机位数据继续验证的边界样本。2026-09-05 已完成 easySVA 平台端到端验收：持续睡岗触发告警，正常写字未误报，Java 后端保存告警记录、截图和视频，原有 `on_yolo11n_80` 布控仍可产生普通行为告警。
...
| 本地视频输出结果 | 已完成首轮基线 | 公开小样本用于可执行性和边界验证；P04 仍需目标机位样本复核 |
| ONNX 导出与 C++ 推理 | 已完成 | 输入 `[1,3,640,640]`，输出 `[1,56,8400]`，CPU 冒烟测试通过 |
| 布控可选择睡岗 | 已完成 | 平台可选 `on_yolo11n_pose`、目标 `person` 和“睡岗”行为规则 |
| Analyzer 产生真实睡岗告警 | 已完成 | 持续睡岗视频在真实平台任务中触发告警 |
| Java 入库和截图 | 已完成 | `SVA_SLEEP` 记录、截图和 ZLM 视频均可从平台访问 |
| 原有 YOLO 不受影响 | 已完成 | `on_yolo11n_80` 任务产生普通“停留告警”；资源调整后 Analyzer 稳定运行 |
