...
导出参数为固定尺寸、`nms=False`。C++ 运行时只接受以下两种三维布局：`[1, 56, N]` 或 `[1, N, 56]`；`[1, 56, 8400]` 是当前基线模型的实测示例，不是所有权重的硬编码尺寸：
...
1. 使用独立的 `AlgorithmOnYoloPose`，按 ONNX Runtime 实际读取的节点和维度解析 Pose 输出；不要让通用检测解码器兼容两种张量语义。（已完成）
2. 扩展检测结果结构，使每个人框携带 17 个 `(x, y, confidence)`，并通过事件 JSON 输出。（已完成）
3. 确认人框 NMS 后关键点仍与对应人物保持同一索引。（已完成）
4. 把 track ID、时间戳、Pose 特征放入每路视频、每个布控独立的时序上下文。（已完成）
5. 移植 NORMAL/SUSPECT/SLEEP/RECOVER 状态机并读取规则参数。（已完成）
6. 告警沿用现有 `sleep` 通道，并在对象 JSON 中输出状态、证据和特征。（已完成；布控配置需与告警记录一同留档）
