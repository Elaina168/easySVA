...
1. **请求结构与校验**：
   ...
   ```
   `controlCode="*"` 表示全部运行中的布控；具体编号只修改该布控的启用睡岗规则。目标检出置信度和 NMS 阈值属于共享 Pose 模型参数，只能随 `*` 下发。
2. **多线程安全分发**：
   - C++ Analyzer 主服务 `Server` 监听 `POST /api/control/update-algorithm-config`；
   - `Server` 先校验字段类型、取值范围和确认窗口单位，再调用 `Scheduler::updateAlgorithmConfig`；
   - `Scheduler` 持有任务容器互斥锁，`Worker` 持有运行布控容器互斥锁，只更新实际存在且启用睡岗规则的布控；
   - 至少一个睡岗布控更新成功后，才修改共享 Pose 模型阈值；模型阈值使用原子变量在每帧一致读取；
   - 热调只作用于当前运行实例，C++ 返回 `code=1000`、`updatedControls` 和 `globalThresholdsUpdated`，Java 再核验回执后返回后端 `code=200`。
