...
- 前端在动态启用算法流失败时，回退到布控详情中已有的算法流地址；如果该地址不可用，再回退到设备原始预览地址。
...
布控页面获取任务详情后，首先尝试调用 `live-output`。调用成功时使用接口返回的 `algorithmStreamUrl`；调用失败时不再中断整个播放流程，而是检查任务详情中的 `pushEnabled` 和 `algorithmStreamUrl`。当任务已启用推流且地址存在时，直接播放该算法流；否则调用设备预览接口获取原始视频地址。
...
- `POST /deployments/{id}/live-output` 即使返回 404，也不会阻断播放地址解析。
