...
- **算法路由接入**：[Scheduler.cpp](file:///home/sugaria/easySVA-Mo/SVA-server/Analyzer/Core/Scheduler.cpp) 中完成 `sleep_yolopose` 与 `on_sleep_yolopose` 算法编号注册与动态路由分发。
- **ONNX Runtime 引擎加载**：
...
- **姿态关键点与睡岗逻辑判定**（[SleepPoseEvaluator.cpp](file:///home/sugaria/easySVA-Mo/SVA-server/Analyzer/Core/SleepPoseEvaluator.cpp)）：
...
- **统一地址规范化（`normalizeStreamUrl`）**（[AvPullStream.cpp](file:///home/sugaria/easySVA-Mo/SVA-server/Analyzer/Core/AvPullStream.cpp)）：
...
- **API 入参解析健壮性**（[Server.cpp](file:///home/sugaria/easySVA-Mo/SVA-server/Analyzer/Core/Server.cpp)）：
...
### 3. 前端监控台与流媒体长连接优化
- **流媒体协议解析与自动回收缓存**：
  - [RTSPPlayer/index.vue](file:///home/sugaria/easySVA-Mo/SVA-web/src/components/RTSPPlayer/index.vue) 与 [realtime.vue](file:///home/sugaria/easySVA-Mo/SVA-web/src/views/device/realtime.vue) 统一支持国标地址转换低延迟 FLV 流播放。
